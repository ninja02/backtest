...
def handle_sell_list(tradingDay, dailyQuote, stockTradeList, currHoldSet, capitalEntity, actualSellListMV):
    ...
    for actualDict in actualSellListMV:
        ...
        #部分卖出
            ...
            stockHoldEntity = currHoldSet.get(innerCode)
            #当前市值
            currMV = BackTestHelper.get_sell_mv(stockHoldEntity.buyPrice, stockHoldEntity.buyMV, sellPrice)
            #卖掉部分后的市值
            currMV = currMV - sellMV
            #更新持仓(价格，仓位，变更日)
            stockHoldEntity.set_buy_price(sellPrice)
            stockHoldEntity.set_buy_mv(currMV)
        ...
        stockTradeDict = {'tradingDay': dateStr, 'innerCode': innerCode, 'type': -1, 'price': sellPrice,
                          ...
                          'sellCash': sellCash,'buyCash':'','openDate':openDate}
        stockTradeList.append(stockTradeDict)
...
def trade_func1(curr_select_stock, curr_hold_set, daily_quote, trading_day, signal_data, capital_entity, daily_quote_pn, daily_quote1day, last_capital_entity):
    ...
    #计算计划买入卖出仓位（去掉停牌）
    plan_buy_list_mv = get_plan_buy_list_mv(plan_buy_list, daily_quote, trading_day, cannot_buy_list, daily_quote_pn, daily_quote1day, last_capital_entity, curr_select_stock)
    t5 = time.time()
    plan_sell_list_mv = get_plan_sell_list_mv(plan_sell_list, trading_day, signal_data, daily_quote, cannot_sell_list, daily_quote1day, curr_hold_set)
    t6 = time.time()
    ...
    # print("dateStr:" + DateUtil.datetime2Str(tradingDay) + " planBuyList:" + str(len(planBuyList)) + " planSellList:" + str(len(planSellList)) +
    #       " prevHoldList:" + str(len(prevHoldList)) + " planBuyListMV:" + str(len(planBuyListMV))  + " planSellListMV:" + str(len(planSellListMV))  +
    #       " forceSellListMV:" + str(len(forceSellListMV))  +
    #       " actualBuyList:" + str(len(actualBuyList)) + " actualSellList:" + str(len(actualSellList)) +
    #       " usableCash:" + str(capitalEntity.getUsableCash()) + " stockMV:" + str(capitalEntity.getStockMV()) + " totalAsset:" + str(capitalEntity.getTotalAsset())
    # )
