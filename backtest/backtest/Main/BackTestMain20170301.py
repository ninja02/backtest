...
def handleBuyList(tradingDate,buyList,dailyQuote,usableVol,stockHoldDF,stockTradeList,currHoldSet,actualBuyList,cannotBuyList,capitalEntity):
    ...
    for innerCode in buyList:
        ...
        if buyFlg != -1:
            ...
            stockTradeDict = {'tradingDate':tradingDate,'innerCode':innerCode,'type':1,'vol':vol,'price':buyPrice,'turnover':turnover,'commission': commission}
...
def getToSellInSellList(sellList,tradingDate,signalData,dailyQuote,numOfToKeepInSellList,cannotSellList,currHoldSet):
    ...
    dic = {'innerCode': sellList}
    sortedSellDf = pd.DataFrame(data=dic)
    sortedSellDf.insert(1, 'Mom', Series())
    #print(sortedSellList)
    #从信号数据中获取Mom
    for innerCode in sellList:
        entity = signalData.ix[(tradingDate, innerCode)]
        sortedSellDf.loc[(sortedSellDf['innerCode'] == innerCode), ['Mom']] = entity['Mom']
    #按Mom倒序排序
    sortedSellDf = sortedSellDf.sort_values(by='Mom', ascending=False)
    ...
    #去掉需要保留的股票
    toSellInSellList = sortedSellDf[~(sortedSellDf.innerCode.isin(toKeepInSellList))]['innerCode'].values
...
def handleSellList(tradingDate,sellList,dailyQuote,usableVol,stockHoldDF,stockTradeList,currHoldSet,actualSellList,cannotSellList,
