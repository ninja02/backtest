...
#signalDataAddr,databaseDict,signalColumnDict,
def initial_func(dailyQuoteAddr,indexQuoteAddr,benchmark, doPlot,global_dict):
    ...
    global_dict['dailyQuote']=dailyQuote
    global_dict['indexQuote']=indexQuote
    global_dict['benchmark']=benchmark
    global_dict['doPlot']=doPlot
    global_dict['maxDailyQuoteDate']=maxDailyQuoteDate
    global_dict['minDailyQuoteDate']=minDailyQuoteDate
    global_dict['maxIndexQuoteDate']=maxIndexQuoteDate
    global_dict['minIndexQuoteDate']=minIndexQuoteDate

    return global_dict

def get_start_end_date(global_dict):
    maxSignalDate = global_dict['maxSignalDate']
    minSignalDate = global_dict['minSignalDate']
    maxDailyQuoteDate = global_dict['maxDailyQuoteDate']
    minDailyQuoteDate = global_dict['minDailyQuoteDate']
    maxIndexQuoteDate = global_dict['maxIndexQuoteDate']
    minIndexQuoteDate = global_dict['minIndexQuoteDate']
    ...
    global_dict['startDate'] = startDate
    global_dict['endDate'] = endDate

    return global_dict

#主程序
# @TimeUtil.checkConsumedTime6
def main(read_func,select_stock_func,trade_func_main,techList,sliceDict,signalDataAddr = '', dailyQuoteAddr = '', indexQuoteAddr='', benchmark='',
    ...
    global_dict = {}
    #signalDataAddr, databaseDict, signalColumnDict,
    global_dict = initial_func(dailyQuoteAddr, indexQuoteAddr, benchmark, doPlot, global_dict)

    global_dict = read_func(signalDataAddr, databaseDict, signalColumnDict, global_dict)

    #select top 5 group by TradingDay order by Mom desc
    global_dict = select_stock_func(techList, sliceDict, global_dict)
    # print('allSelectStock:'+str(len(allSelectStock)))
    # # print(allSelectStock)
    # if len(allSelectStock) == 0:
    #     return

    global_dict = get_start_end_date(global_dict)

    global_dict = trade_func_main(global_dict)

    global_dict = performance_func(global_dict)
...
def performance_func(global_dict):
    start_stat = time.clock()

    # print(global_dict)
    doPlot = global_dict['doPlot']
    stockStatDailyDf = global_dict['stockStatDailyDf']
    ...
    global_dict['yearDf'] = yearDf
    global_dict['maxdrop'] = maxdrop
    global_dict['sharpRatio'] = sharpRatio
    global_dict['sharpRatioDf'] = sharpRatioDf
    global_dict['result'] = result
...
#外部调用
def trade_func_main1(global_dict):
    return trade_func_main_private(global_dict, trade_func1)

#非外部调用
def trade_func_main_private(global_dict,trade_func):

    dailyQuote = global_dict.get('dailyQuote')
    indexQuote = global_dict.get('indexQuote')
    startDate = global_dict.get('startDate')
    endDate = global_dict.get('endDate')

    allSelectStock = global_dict.get('allSelectStock')
    signalData = global_dict.get('signalData')
    benchmark = global_dict.get('benchmark')
    # trade_func = global_dict.get('trade_func')
    ...
    stockStatDailyDf = pd.DataFrame(index=pd.date_range(startDate, endDate),
                                    columns=['netValue', 'changePCT', 'buyCnt', 'sellCnt', 'prevHoldCnt', 'currHoldCnt',
                                             'cannotSellCnt', 'cannotBuyCnt',
                                             'usableCash', 'totalAsset', 'indexChangePCT', 'relativeChangePCT',
                                             'relativeNetValue'])
    # 初始化每日持仓表
    stockHoldDailyList = []
    stockTradeList = []
    ...
    for index, item in enumerate(dateList):
        ...
        dict = trade_func(currSelectStock, currHoldSet, dailyQuote, tradingDay, signalData, capitalEntity, dailyQuotePn,
                          dailyQuote1day, lastCapitalEntity)
        ...
        # 统计表
        stockStatDailyDf.ix[
            tradingDayStr] = netValue, dailyChangePCT, buyCnt, sellCnt, prevHoldCnt, currHoldCnt, cannotSellCnt, cannotBuyCnt, usableCash, totalAsset, indexChangePCT, relativeChangePCT, relativeNetValue
        # 昨日资产
        lastCapitalEntity = copy.copy(capitalEntity)
        # 昨日相对净值
    ...
    global_dict['stockStatDailyDf'] = stockStatDailyDf
...
def select_stock_func1(tech_list, slice_dict, global_dict):
    print('select_stock_func1')

    signalData = global_dict['signalData']

    numOfDailySignal = slice_dict.get('numOfDailySignal')

    allSelectStock = signalData.groupby(level=StockConst.TRADINGDAY).apply(SelectUtil.top, numOfDailySignal, tech_list[0], False) #False True

    global_dict['allSelectStock'] = allSelectStock
...
#numOfDailySignal：每天选股个数
@TimeUtil.check_consumed_time7
def select_stock_func2(tech_list, slice_dict, global_dict):
    ...
    allSelectStock = signalData.groupby(level=StockConst.TRADINGDAY).apply(SelectUtil.top, numOfDailySignal, tech_list[0], True) #False True
...
#2因子加权重取前N
@TimeUtil.check_consumed_time7
def select_stock_func3(tech_list, slice_dict, global_dict):
    ...
    allSelectStock = signalData.groupby(level=StockConst.TRADINGDAY).apply(SelectUtil.top_with_weight, numOfDailySignal, tech_list, False, SelectUtil.handle_na1) #False True  StockConst.Mom,'Momp'
...
#选股方法4:
@TimeUtil.check_consumed_time7
def select_stock_func4(tech_list, slice_dict, global_dict):
    print('select_stock_func4')

    signalData = global_dict['signalData']

    # numOfDailySignal = 4
    allSelectStock = signalData.groupby(level=StockConst.TRADINGDAY).apply(SelectUtil.top_with_slice, slice_dict, tech_list[0], False) #False True
...
#选股方法5:仓位权重
@TimeUtil.check_consumed_time7
def select_stock_func5(tech_list, slice_dict, global_dict):
    print('select_stock_func5')

    signalData = global_dict['signalData']

    num_of_daily_signal = slice_dict.get('numOfDailySignal')
    frm_gb = signalData.groupby(level=StockConst.TRADINGDAY)

    allSelectStock = frm_gb.apply(SelectUtil.top_with_vol_weight, num_of_daily_signal, tech_list[0], False) #False True

    global_dict['allSelectStock'] = allSelectStock
    # return allSelectStock
    return global_dict

# def test123(techList, sliceDict, globalDict):
#     print('test123')
#     return globalDict
...
def read_func1(signalDataAddr, databaseDict, signalColumnDict, global_dict):
    ...
    global_dict['signalData'] = signalData
    global_dict['maxSignalDate'] = maxSignalDate
    global_dict['minSignalDate'] = minSignalDate
    # return signalData
    return global_dict
