...
def getActualBuyList(planBuyList,dailyQuote,tradingDate,cannotBuyList,dailyQuotePn, dailyQuote1day):
...
def handleBuyList(tradingDate,dailyQuote,stockTradeList,currHoldSet,capitalEntity,actualBuyList):
    ...
    for innerCode in actualBuyList:
        ...
        #当前持仓
        stockHoldEntity = StockHoldEntity.StockHoldEntity(innerCode, buyPrice, cost, DateUtil.datetime2_str(tradingDate), buyMV, buyPrice, buyMV)
...
def main(select_stock_func,trade_func,techList,sliceDict,signalDataAddr = '', dailyQuoteAddr = '', startDate = '2004-01-01', endDate = '2005-12-31'):
    start = time.clock()
    ...
    groupedSignalData = select_stock_func(signalData,techList,sliceDict)
    if len(groupedSignalData) == 0:
        return

    #当前持仓 key:innerCode value:
    currHoldSet = {}
    #资金情况
    capitalEntity = CapitalEntity.CapitalEntity(initialMV,0,0)
    lastMV = initialMV
    #初始化每日统计表
    ...
    #for date in dateList:
        ...
        #print(dateStr)
        currSignalData = groupedSignalData.xs(signalDay)
        ...
        dict = trade_func(currSignalData, currHoldSet, dailyQuote, tradingDay, signalData, capitalEntity, dailyQuotePn, dailyQuote1day)
...
def processBySlice(select_stock_func,trade_func,techList,sliceTotalNum,numOfDailySignal,signalDataAddr = '', dailyQuoteAddr = '', startDate = '2004-01-01', endDate = '2005-12-31'):
...
def select_stock_func4(signalData,techList,sliceDict):
    ...
    # numOfDailySignal = 4
    groupedSignalData = signalData.groupby(level=StockConst.TRADINGDAY).apply(SelectUtil.top_with_slice, sliceDict, techList[0], False) #False True
    return groupedSignalData

#选股方法5:仓位权重
@TimeUtil.check_consumed_time7
def select_stock_func5(signalData,techList,sliceDict):
    print('select_stock_func5')

    numOfDailySignal = sliceDict.get('numOfDailySignal')

    groupedSignalData = signalData.groupby(level=StockConst.TRADINGDAY).apply(SelectUtil.top_with_vol_weight, numOfDailySignal, techList[0], False) #False True
    return groupedSignalData

#交易方法1
#numOfDailySignal,
#算出实际买入和卖出的股票
def trade_func1(currSignalData, currHoldSet, dailyQuote, tradingDay, signalData, capitalEntity, dailyQuotePn, dailyQuote1day):
    # 计划买入列表
    t1 = time.time()
